...
const completeProfileSetup = async (req, res) => {
  try {
    const { fullName, businessName, country, logo, preferredCurrency, defaultServiceRate } = req.body
...
const addFirstClient = async (req, res) => {
  try {
    const { name, email, phone, tags, serviceType, customRate, notes } = req.body
