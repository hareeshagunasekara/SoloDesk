...
const { convertToUserTimezone, calculateTimeDifference } = require('../utils/timezoneUtils')
...
userSchema.virtual('formattedLastLogin').get(function() {
  ...
  if (this.country) {
    try {
      const { diffInHours, diffInDays } = calculateTimeDifference(this.lastLogin, this.country);
