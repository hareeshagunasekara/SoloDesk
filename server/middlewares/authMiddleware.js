...
const auth = async (req, res, next) => {
  try {
    ...
    console.log(
      "Auth middleware - authorization header (lowercase):",
      authHeaderAlt ? `"${authHeaderAlt}"` : "missing",
    );

    const token =
      authHeader?.replace("Bearer ", "") ||
      authHeaderAlt?.replace("Bearer ", "");
    console.log(
      "Auth middleware - Token extracted:",
      token ? `"${token.substring(0, 50)}..."` : "missing",
    );
    ...
    console.log(
      "Auth middleware - JWT_SECRET length:",
    ...
    if (!process.env.JWT_SECRET) {
      console.warn(
        "Auth middleware - JWT_SECRET not set, using fallback secret for development.",
      );
    }
